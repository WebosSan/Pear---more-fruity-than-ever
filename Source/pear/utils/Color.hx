...
abstract Color(Int) from Int from UInt to Int to UInt
{
	...
	public static function gradient(Color1:Color, Color2:Color, Steps:Int, ?Ease:Float->Float):Array<Color>
	{
		...
		if (Ease == null)
		{
			Ease = function(t:Float):Float return t;
		}
	...
	{
		return fromHSB(wrap(Std.int(hue) + 180, 0, 360), brightness, saturation, alphaFloat);
	}
	...
	{
		var warmer:Int = fromHSB(wrap(Std.int(hue) - Threshold, 0, 360), saturation, brightness, alphaFloat);
		var colder:Int = fromHSB(wrap(Std.int(hue) + Threshold, 0, 360), saturation, brightness, alphaFloat);
	...
	{
		var oppositeHue:Int = Std.int(wrap(Std.int(hue) + 180, 0, 360));
		var warmer:Color = fromHSB(wrap(oppositeHue - Threshold, 0, 360), saturation, brightness, alphaFloat);
		var colder:Color = fromHSB(wrap(oppositeHue + Threshold, 0, 360), saturation, brightness, alphaFloat);
	...
	{
		var triadic1:Color = fromHSB(wrap(Std.int(hue) + 120, 0, 360), saturation, brightness, alphaFloat);
		var triadic2:Color = fromHSB(wrap(Std.int(triadic1.hue) + 120, 0, 360), saturation, brightness, alphaFloat);
	...
	public function getColorInfo():String
	{
		...
		// HSB/HSL info
		result += "Hue: " + roundDecimal(hue, 2) + " Saturation: " + roundDecimal(saturation, 2) + " Brightness: "
			+ roundDecimal(brightness, 2) + " Lightness: " + roundDecimal(lightness, 2);
	 ...
	 * @return	This color
	 */
	public inline function setHSB(Hue:Float, Saturation:Float, Brightness:Float, Alpha = 1.0):Color
	{
	 ...
	 * @return	This color
	 */
	public inline function setHSL(Hue:Float, Saturation:Float, Lightness:Float, Alpha = 1.0):Color
	{
	...
	public function alphaBlend(dst:Color):Color {
		var src:Color = this;
	
		var sr:Float = src.red;
		var sg:Float = src.green;
		var sb:Float = src.blue;
		var sa:Float = src.alphaFloat; 
	
		var dr:Float = dst.red;
		var dg:Float = dst.green;
		var db:Float = dst.blue;
		var da:Float = dst.alphaFloat; 
		...
		var outA:Float = sa + da * (1 - sa);
		if (outA <= 0) return Color.fromRGB(0, 0, 0, 0);
	
		var outR:Float = (sr * sa + dr * da * (1 - sa)) / outA;
		var outG:Float = (sg * sa + dg * da * (1 - sa)) / outA;
		var outB:Float = (sb * sa + db * da * (1 - sa)) / outA;
	
		return Color.fromRGB(
			Std.int(outR < 0 ? 0 : outR > 255 ? 255 : outR),
			Std.int(outG < 0 ? 0 : outG > 255 ? 255 : outG),
			Std.int(outB < 0 ? 0 : outB > 255 ? 255 : outB),
			Std.int(outA * 255)
		);
	}
	...
	// Helper functions to replace FlxMath dependencies
	static inline function bound(Value:Float, Min:Float, Max:Float):Float
	{
		return Value < Min ? Min : Value > Max ? Max : Value;
	}

	static inline function wrap(Value:Float, Min:Float, Max:Float):Float
	{
		var range = Max - Min + 1;
		if (Value < Min) Value += range * Math.ceil((Min - Value) / range);
		return Min + (Value - Min) % range;
	}

	static inline function roundDecimal(Value:Float, Precision:Int):Float
	{
		var mult:Float = 1;
	...
	public static inline function random(?alpha:Int = 255):Color {
		var r:Int = Std.random(256);
		var g:Int = Std.random(256);
